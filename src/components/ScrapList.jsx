...
const ScrapList = () => {
  ...
  const [filterDate, setFilterDate] = useState("all"); // 날짜 필터 상태 ('today', 'week', 'month', 'all')
  ...
  const filteredScraps = scraps.filter(scrap => {
    // 1. 검색어 필터: 제목이나 요약 내용에 검색어가 포함되어 있는지 확인
    const matchesSearch = 
      scrap.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      scrap.fullSummary?.toLowerCase().includes(searchTerm.toLowerCase());
    
    // 2. 날짜 필터: 선택된 기간(오늘, 1주일, 1달)에 포함되는지 확인
        ...
        <div className="flex-between">
          <div style={{ display: "flex", gap: "8px", overflowX: "auto" }}>
            {["all", "today", "week", "month"].map(f => (
              ...
              >
                {f === "all" ? "전체" : f === "today" ? "오늘" : f === "week" ? "일주일" : "한달"}
              </button>
            ))}
          </div>
